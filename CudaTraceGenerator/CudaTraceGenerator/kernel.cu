...
__global__ void counterKernel(int* trace, bool *work, int offset)
{
    int counter = 0;
    while (*work)
    {
...
int main()
{
    const unsigned int attackLength = 15;
    const unsigned int P = 1;
    const unsigned int traceLength = attackLength * 1000 / P;
...
cudaError_t countWithCuda(int* trace, const unsigned int attackLength, const unsigned int traceLength, const unsigned int P)
{
    ...
    *hostWork = true;
    ...
    cudaStatus = cudaMalloc((void**)&devTrace, traceLength * sizeof(int));
    if (cudaStatus != cudaSuccess) {
    ...
    while (attackLength * 1000 >= hostTime - attackTimeout)
    {
        ...
        counterKernel <<<1, 1, 0, execStream>>> (devTrace, devWork, offset);
