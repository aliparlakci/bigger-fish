...
parser = argparse.ArgumentParser(
    description='Automate the collection of video player based CPU traces.')
parser.add_argument("--len", type=int, default=15,
                    help="The trace length for the recordings in seconds.")
parser.add_argument("--samples", type=int, default=5,
                    help="The number of runs for each video file.")
parser.add_argument("--out_dir", type=str, default="",
                    help="The output location.")
parser.add_argument(
    "--codecs", choices=["3gp", "flv", "mp4", "mkv", "*"], nargs="+")
parser.add_argument(
    "--browsers", choices=["chrome", "safari", "edge", "firefox", "*"], nargs="+")
parser.add_argument(
    "--players", choices=["mpv", "mplayer", "vlc", "*"], nargs="+")
...
def run(file_path, trace_length, player_type, browser="CHROME"):
    with TraceCollector(trace_length=trace_length) as collector:
        ...
        player_thread = threading.Thread(
            target=lambda: player.play(file_path, trace_length))
        player_thread.start()

        trace = collector.collect_traces()
        player_thread.join()
        return [trace, file_path, browser.upper(), player_type.name.upper(), platform.platform(), os.getlogin(), int(time.time())]
...
def main():
    ...
    players = ["mpv", "mplayer",
               "vlc"] if "*" in opts.players else opts.players
    codecs = ["3gp", "flv", "mp4",
              "mkv"] if "*" in opts.codecs else opts.codecs
    browsers = ["chrome", "edge",
                "firefox"] if "*" in opts.browsers else opts.browsers
    ...
    for codec, player_name, browser in tqdm(combinations(players, codecs, browsers)):
        ...
        out_file = os.path.join(
            out_dir, fr"traces_{num_of_samples}_runs_{trace_len}_sec_{codec}_{player.name}_{browser}_{sys.platform}_{os.getlogin()}_{int(time.time())}.pkl")
        traces = []
